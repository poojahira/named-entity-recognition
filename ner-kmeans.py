...
embeddings = loadGloveModel('glove.6B.50d.txt')
l = np.asarray(l)
...
def extract_training_data(labeled_data,filename,google_filename1,google_filename2,google_filename3):
	...
	with open(filename) as file:
		...
		for line in file:
			...
			if (prev_tag == 'I-ORG' and (next_pos == 'NNP' or next_pos == 'NNPS') and data[0] in ['of','for','and']) or data[1] == 'NNP' or data[1] == 'NNPS' or hyphen_flag == 'True' or (data[0][0].isupper() and data[1] in ['NN','NNS','JJ'] and '-' not in data[0]):
				hyphen_flag = 'False'
				if data[0].lower() in embeddings:
					cluster.append(embeddings[data[0].lower()])
					token_list.append(data[0])
				else:
					cluster.append(embeddings['<unk>'])
					token_list.append(data[0])
...
def run_kmeans(X,n):
	cluster = np.asarray(X)
	kmeans = KMeans(n_clusters=n,random_state = 0).fit(cluster)
	kmeans_labels = kmeans.labels_
	kmeans_centroids = kmeans.cluster_centers_
	for i,word in enumerate(token_list):
		cluster_centroid[word] = kmeans_centroids[kmeans_labels[i]]
	return kmeans

def build_train_set(labeled_data,kmeans):
	train_set = [(feature_builder(prev_tag,prev_word,prev_pos,prev_chunk,token,pos,chunk,next_word,next_pos,next_chunk,google_category,google_category2,google_category3,train,kmeans),tag) for (prev_tag,prev_word,prev_pos,prev_chunk,token,pos,chunk,tag,next_word,next_pos,next_chunk,google_category,google_category2,google_category3,train) in labeled_data]
	return train_set

def feature_builder(prev_tag,prev_word,prev_pos,prev_chunk,token,pos,chunk,next_word,next_pos,next_chunk,google_category,google_category2,google_category3,train,kmeans):
	features = {}
	hyphen_flag = 'False'
	word = []
	kmeans_centroids = kmeans.cluster_centers_
	...
	if pos == 'NNP' or pos == 'NNPS' or hyphen_flag == 'True' or (token[0].isupper() and pos in ['NN','NNS','JJ'] and '-' not in token):
		...
		if token.lower() in embeddings:
			dimensions = embeddings[token.lower()]
		else:
			dimensions = embeddings['<unk>']
		for i,d in enumerate(dimensions):
			key = 'd' + str(i)
			features[key] = dimensions[i]
		'''
		hyphen_flag = 'False'
		...
		if train == 'True':
			cluster1 = cluster_centroid[token]
			for i in range(0,50):
				key = 'c' + str(i)
				features[key] = cluster1[i]
		else:
			if token.lower() in embeddings:
				dimensions = embeddings[token.lower()]
			else:
				dimensions = embeddings['<unk>']
			cluster1 = kmeans_centroids[kmeans.predict([dimensions])]
			for i in range(0,50):
				key = 'c' + str(i)
				features[key] = cluster1[0][i]
			

		'''
		if prev_word.lower() in uplus:
			udimensions = uplus[prev_word.lower()]
			bdimensions = bminus[prev_word.lower()]
		else:
			udimensions = uplus['<unk>']
			bdimensions = bminus['<unk>']
		for i,d in enumerate(udimensions):
			ukey = 'u_prev_word' + str(i)
			bkey = 'b_prev_word' + str(i)
			features[ukey] = udimensions[i]
			features[bkey] = bdimensions[i]
		
		if next_word.lower() in uplus:
			udimensions = uplus[next_word.lower()]
			bdimensions = bminus[next_word.lower()]
		else:
			udimensions = uplus['<unk>']
			bdimensions = bminus['<unk>']
		for i,d in enumerate(udimensions):
			ukey = 'u_next_word' + str(i)
			bkey = 'b_next_word' + str(i)
			features[ukey] = udimensions[i]
			features[bkey] = bdimensions[i]
		'''
...
def process_test_data(classifier,filename,google_filename1,google_filename2,google_filename3,google_filename4,kmeans):
	i = 0
	...
	with open(filename) as file:
		...
		for line in file:
			...
			if line != '\n':
				...
				prev = next(file,False)
				count += 2
			featureset = feature_builder(prev_tag,prev_word,prev_pos,prev_chunk,data[0],data[1],data[2].strip("\n"),next_word,next_pos,next_chunk,google_category,google_category2,google_category3,train,kmeans)
...
if __name__=='__main__':
	...
	f = open('models/classifier-binarizer-glove-clustering-1000.pickle','rb')
	...
	kmeans = run_kmeans(cluster,1000)
	
	#labeled_data = extract_training_data(labeled_data,"CONLL_dev.pos-chunk-name","dev_google.txt","dev_google_2words.txt","dev_google_3words.txt",kmeans)
	train_set = build_train_set(labeled_data,kmeans)
	#classifier = nltk.MaxentClassifier.train(train_set,trace = 3,max_iter = 100)
	#pickle.dump(classifier , f)
	f.close()
	process_test_data(classifier,'data/CONLL_dev.pos-chunk',"google/dev_google.txt","google/dev_google_2words.txt","google/dev_google_3words.txt","google/dev_google_last_ditch.txt",kmeans)
	#process_test_data(classifier,'CONLL_test.pos-chunk',"test_google.txt","test_google_2words.txt","test_google_3words.txt","test_google_last_ditch.txt")
